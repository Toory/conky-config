...
if cmus-remote -Q &>/dev/null ; then
        ...
        FILE=$( ls "$FOLDER" | grep -iw -m1 'folder.jpg\|cover.jpg\|front.jpg\|folder.jpeg\|cover.jpeg\|front.jpeg\|folder.png\|cover.png\|front.png' )
	...
	if [ -z "$FILE" ]; then
		FILE=$( ls "$FOLDER" | grep -i -m1 '.jpg\|.png\|jpeg' )
	fi
